...
namespace Nhom15_QuanLyThongTinSV.UC_Control
{
    public partial class uc_XemDS : UserControl
    {
        ...
        public uc_XemDS(string MaGV, string hoten)
        {
            InitializeComponent();
            this.maGV = MaGV;
            this.GiangVien = hoten;
        }
        ...
        private void DemSoHocPhanGV(string tenGV)
        {
            lblSiso.Visible = true;
            if (dgv_QLHP.Rows.Count == 0)
            {
                MessageBox.Show("Không có dữ liệu học phần.");
                return;
            }

            HashSet<string> hocKyNamHocSet = new HashSet<string>();

            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                string queryHK = "SELECT DISTINCT HocKy, NamHoc FROM HocPhan WHERE GiangVien = @GiangVien";
                using (SqlCommand cmd = new SqlCommand(queryHK, conn))
                {
                    cmd.Parameters.AddWithValue("@GiangVien", tenGV);
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            string hocKy = reader["HocKy"].ToString();
                            string namHoc = reader["NamHoc"].ToString();
                            hocKyNamHocSet.Add(hocKy + "|" + namHoc);
                        }
                    }
                }
            }
            ...
            {
                conn.Open();
                string query = "SELECT dbo.fn_DemSoHPGV(@GiangVien, @HocKy, @NamHoc)";
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    cmd.Parameters.Add("@GiangVien", SqlDbType.NVarChar);
                    cmd.Parameters.Add("@HocKy", SqlDbType.NVarChar);
                    cmd.Parameters.Add("@NamHoc", SqlDbType.NVarChar);

                    string resultMessage = "";

                    foreach (string hkNh in hocKyNamHocSet)
                    {
                        string[] parts = hkNh.Split('|');
                        string hocKy = parts[0];
                        string namHoc = parts[1];

                        cmd.Parameters["@GiangVien"].Value = GiangVien;
                        cmd.Parameters["@HocKy"].Value = hocKy;
                        cmd.Parameters["@NamHoc"].Value = namHoc;

                        object result = cmd.ExecuteScalar();
                        int soHocPhan = (result != DBNull.Value) ? Convert.ToInt32(result) : 0;

                        resultMessage += $"{hocKy}-{namHoc}: {soHocPhan} HP; ";
                    }
                    lblSiso.Text = $"GV {maGV} có {resultMessage.TrimEnd(';', ' ')}";
                }
            }
        ...
        private void dgv_QLHP_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0 && dgv_QLHP.Columns[e.ColumnIndex].Name == "btnXem")
            {
                ...
                this.maHP = maHP;
                string sql = "SELECT * FROM dbo.fn_XemSinhVienTrongLop(@MaHP)";
                try
                {
                    using (SqlConnection conn= new SqlConnection(connectionString))
                    using (SqlCommand cmd = new SqlCommand( sql, conn))
