...
namespace Nhom15_QuanLyThongTinSV.UC_Control
{
    public partial class uc_Tracuuhocphan : UserControl
    {
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            string query = "SELECT * FROM dbo.fn_TraCuuHocPhan(@MaHP, @TenHP, @HocKy)";
            ...
            foreach (DataGridViewRow row in dgv_tracuuhocphan.Rows)
            {
                if (row.Cells["MaHP"].Value != null)
                {
                    string maHPRow = row.Cells["MaHP"].Value.ToString();
                    int soLuong = HienSiSo(maHPRow);
                    row.Cells["SoLuong"].Value = soLuong;
                }
            }

        }
        private int HienSiSo(string maHP)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                string sql = "SELECT dbo.fn_SoLuongSVDangKy(@MaHP)";
