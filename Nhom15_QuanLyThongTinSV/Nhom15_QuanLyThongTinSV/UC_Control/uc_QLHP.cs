...
namespace Nhom15_QuanLyThongTinSV.UC_Control
{
    public partial class uc_QLHP : UserControl
    {
        ...
        {
            string sql = "SELECT * FROM fn_LayAllDSHP();";
            try
            {
                using (SqlConnection conn= new SqlConnection(connectionString))
                using (SqlCommand cmd = new SqlCommand(sql, conn))
        ...
        private void dgvQLHP_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0) 
            {
                txtMaHP.TextChanged -= txtMaHP_TextChanged;
        ...
        {
            string filterText = txtMaHP.Text.Trim();

            if (dgvQLHP.DataSource is DataTable dt)
            {
                ...
                {
                    dv.RowFilter = $"MaHP LIKE '%{filterText}%'";
                }
            }
        }
    }
